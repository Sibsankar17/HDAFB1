...
hospitalName <- function(data, col_num, state) {
...
best <- function(state, outcome) {
  ...
  if (!state %in% data$State) {
    ...
    if(outcome == "heart attack") {
      hosp_name <- HospitalName(data, 11, state)
    } else if(outcome == "heart failure") {
      hosp_name <- HospitalName(data, 17, state)
    } else {
      hospital_name <- HospitalName(data, 23, state)
    }
